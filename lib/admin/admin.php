...
class Kamn_Iconlist_Admin {
		...
		function kamn_iconlist_options_page_contextual_help() {
			...
			$screen->add_help_tab( array(
				...
				'id' => 'kamn-iconlist-license',
				'title' => __( 'License', 'kamn-iconlist' ),
				'content' => implode( '', file( KAMN_ICONLIST_ADMIN_DIR . 'help/license.html' ) ),				
			...
			$screen->add_help_tab( array(
				...
				'id' => 'kamn-iconlist-changelog',
				'title' => __( 'Changelog', 'kamn-iconlist' ),
				'content' => implode( '', file( KAMN_ICONLIST_ADMIN_DIR . 'help/changelog.html' ) ),				
		...
		function kamn_iconlist_enqueue_scripts( $hook ) {
			...
			if( $hook === $this->kamn_iconlist_options_page_hook ) {
				...
				/** Load Admin Scripts */
				wp_enqueue_script( 'kamn-iconlist-admin-js-theme-options', esc_url( KAMN_ICONLIST_ADMIN_URI . 'plugin-options.js' ), array( 'jquery' ) );
				
				/** Load Admin Stylesheet */
				wp_enqueue_style( 'kamn-iconlist-admin-css-theme-options', esc_url( KAMN_ICONLIST_ADMIN_URI . 'plugin-options.css' ) );
		...
		function kamn_iconlist_options_validate( $input ) {
			...
			/** Reset Logic */
			if( $input['kamn_iconlist_reset_control'] == 1 ) {
				$input = $default;
			}			
